...
impl KvStore {
  ...
  fn new_log_file(&mut self, gen: u64) -> Result<BufWriteWithPos<File>> {
...
impl KvsEngine for KvStore {
  ...
  /// # Errors
  ///
  /// It returns `KvsError::UnexpectedCommandType` if the given command type
  /// unexpected.
...
/// Create a new log file with given generation number and add the reader to the
/// readers map.
///
...
) -> Result<BufWriteWithPos<File>> {
  let path = log_path(&path, gen);
  let writer = BufWriteWithPos::new(
    OpenOptions::new()
      .create(true)
      .write(true)
      .append(true)
