...
async fn main() {
  if std::env::var("RUST_LOG").is_err() {
    std::env::set_var("RUST_LOG", "key_value_store=debug");
  }
...
async fn kv_set(
  Path(key): Path<String>,
  ContentLengthLimit(bytes): ContentLengthLimit<Bytes, { 5000 * 1024 }>, // ~5mb
  Extension(state): Extension<SharedState>,
) {
...
fn handle_error(error: BoxError) -> Result<impl IntoResponse, Infallible> {
  if error.is::<tower::timeout::error::Elapsed>() {
    return Ok((StatusCode::REQUEST_TIMEOUT, Cow::from("request time out")));
  }
