...
@return On success: rshm.
        On error: NULL.
*/
SRshm *rshm_create(const char *shmname, const char *programname);
...
*/
void rshm_release(SRshm *rshm);
int rshm_destory(SRshm *rshm);
...
@return On success: return the number of bytes write.
        On error: error code below zero is returned.
*/
int rshm_write(SRshm *rshm, const char *buf, size_t len);
